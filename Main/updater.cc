...
Updater::Updater(const QSettings& settings, Database& db, QObject* _parent)
	: QObject(_parent), db_(db), working_dir_(ResourcePath::fromSystemAbsolute(settings.value("working_dir").toString())),
	  repo_url_(settings.value("repo_url").toString()), repo_branch_(settings.value("repo_branch").toString()),
	  mng_(new VersionManager(working_dir_, repo_url_, repo_branch_, db, this))
{
	...
	{
		qWarning() << "Updater interval not given, using default:" << defaultInterval << "s";
	...
	if (!QDir(sourceDir().toSystemAbsolute()).exists())
		throw Exception("Source directory not found: " + sourceDir().toSystemAbsolute());
	if (!QDir(buildDir().toSystemAbsolute()).exists())
		throw Exception("Build directory not found: " + sourceDir().toSystemAbsolute());
	auto currbuild = buildDir(currentVersion().id()).toSystemAbsolute();
	if (!QDir(currbuild).exists())
		throw Exception("Build directory of current version not found: " + currbuild);
	...
	{
		qWarning() << "Updater enabled not given, using default:" << defaultEnabled << "s";
		startSearch();
	}
...
{
	Process* git(Process::gitClone(repo_url_, sourceDir(), {"-n"})); // -n for no checkout
	...
	connect(git, &Process::OnFailure, [](QString output) { qDebug() << "Cloned error;" << output; });
...
Process* Updater::checkout(Version::ID hash)
{
	...
	connect(git, &Process::OnFailure, [](QString output) { qDebug() << "Check out error:" << output; });
	connect(git, &Process::OnFinished, git, &Process::deleteLater);
...
{
	timer_->stop();
	qDebug().nospace() << "Searching for updates... (thread=" << QThread::currentThreadId() << ")";

	Process* gitF = Process::gitFech(sourceDir(), {"origin"});
	Process* gitL = Process::gitLog(sourceDir(), {"--pretty=format:\"%H#%ad\"", "--date=rfc2822", "--max-count=5",
...
void Updater::logSuccess(QString output)
{
	...
	qDebug("origin/%s is %i ahead and 0 behind HEAD", qPrintable(repo_branch_), lines.size());
	...
	for (QString line : lines)
	{
		...
		if (!QRegExp("^[0-9a-f]{40}$").exactMatch(id)) // match SHA-1 160 bit hashes
		{
...
void Updater::logFailure(QString output) { qCritical() << "Git log failed:" << output; }
