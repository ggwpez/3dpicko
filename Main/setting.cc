...
Setting::Setting(Setting::ID id, QString name, Setting::Type type,
    ...
    : id_(id), name_(name), type_(type), description_(description),
      default_value_(default_value), placeholder_(placeholder), min_(min),
      max_(max), step_(step), value_type_(value_type), options_(options),
      max_str_len_(max_str_len), min_str_len_(min_str_len),
...
void Setting::setValue(const QJsonValue &value) {
  ...
  case Type::CHECKBOX: {
    if (!value.toVariant().canConvert<bool>())
      throw Exception(Q_FUNC_INFO, "Expected bool but got " + value.toString());
    else
      value_ = value.toBool();
