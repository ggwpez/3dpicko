...
void VersionManager::checkoutAndQmakeVersion(Version::ID id)
{
	...
	connect(git, &Process::OnStarted, [id]() { qDebug() << "Checking out" << id << "..."; });
...
void VersionManager::qmakeAmdMakeVersion(Version::ID id)
{
	...
	if (!QDir(build_dir.toSystemAbsolute()).exists())
		if (!QDir().mkdir(build_dir.toSystemAbsolute()))
			return emit OnInstallError("qmake (setup phase)", "Cant create build dir" + build_dir.toSystemAbsolute());
...
void VersionManager::makeVersion(Version::ID id)
{
	...
	connect(make, &Process::OnFailure, [this, id](QString output) {
		if (output.contains("internal compiler error"))
		{
			...
			{
				qWarning() << "Compiler crashed (segmentation fault), restarting with" << (max_make_retries_ - make_retries_)
						   << "tries left";
				this->makeVersion(id);
			}
			else
...
void VersionManager::linkVersion(Version::ID id)
{
	...
	connect(ln, &Process::OnFailure, [this, id](QString output) { emit this->OnInstallError("ln", output); });
