...
namespace c3picko
{
VersionManager::VersionManager(ResourcePath working_dir, QString repo_url, QString repo_branch, Database& db, QObject* _parent)
	: QObject(_parent), db_(db), repo_url_(repo_url), repo_branch_(repo_branch), working_dir_(working_dir),
	  selected_(currentVersion().id()), max_interesting_(0)
{
	connect(this, &VersionManager::OnInstallError, this, [this](Version::ID id, QString error) {
...
ResourcePath VersionManager::sourcePath() const { return working_dir_ + "source/"; }
...
void VersionManager::remOldestVersion(Version::ID id)
{
	...
	if (selected_ == id)
		return (void)qWarning("Cant remove selected version");
...
void VersionManager::checkoutAndQmakeVersion(Version::ID id)
{
	...
	connect(git, &Process::OnFailure, [this, id](QString output) { emit this->OnInstallError(id, "git checkout: " + output); });
...
void VersionManager::qmakeAmdMakeVersion(Version::ID id)
{
	...
	if (!QDir(new_build_dir.toSystemAbsolute()).exists())
	{
		if (!QDir().mkdir(new_build_dir.toSystemAbsolute()))
			return emit OnInstallError("qmake (setup phase)", "Cant create build dir" + new_build_dir.toSystemAbsolute());

		// Can we get an older build?
		auto it(std::find(db_.installedVersions().begin(), db_.installedVersions().end(), id));
		if (it != db_.installedVersions().begin())
		{
			--it;
			ResourcePath old_build_dir(working_dir_ + "builds/" + *it);
			// Copy older build to new directory
			if (!copyRecursively(old_build_dir.toSystemAbsolute(), new_build_dir.toSystemAbsolute()))
			{
				// If it did not work, delete it and create a new empty directory
				// (again).
				qWarning() << "Cold not copy build directories";
				QDir(new_build_dir.toSystemAbsolute()).removeRecursively(); // dont check return code
	...
	connect(qmake, &Process::OnFailure, [this, id](QString output) { emit this->OnInstallError("qmake", output); });
...
void VersionManager::makeVersion(Version::ID id)
{
	...
	connect(make, &Process::OnFailure, [this, id](QString output) {
		if (output.contains("internal compiler error"))
		{
			...
			{
				qWarning() << "Compiler crashed (segmentation fault)," << (max_make_retries_ - make_retries_) << "tries left";
				this->makeVersion(id);
			}
			else
				emit this->OnInstallError(id, "Compiler crashed (segmentation fault) too often");
		}
		else
...
{
	ResourcePath s_main(working_dir_ + "main"), s_lqua(working_dir_ + "libquazip.so.1");
	QString		 t_main("builds/" + id + "/Main/Main"), t_lqua("builds/" + id + "/quazip/quazip/libquazip.so.1");

	Process* ln_main(Process::ln(t_main, s_main));
	Process* ln_lqua(Process::ln(t_lqua, s_lqua));

	connect(ln_main, &Process::OnFailure, [this, id](QString output) { emit this->OnInstallError("ln", output); });
	...
	connect(ln_lqua, &Process::OnFailure, [this, id](QString output) { emit this->OnInstallError("ln", output); });
...
void VersionManager::checkoutRepo(Version::ID id)
{
	...
	if (version.state() == Version::State::CLONED || version.state() == Version::State::CHECK_OUT_ERROR)
	{
		transition(version, Version::State::MARKED_FOR_CHECKOUT);
		Process* git(Process::gitCheckout(repo_branch_, working_dir_ + "source/" + id));

		connect(git, &Process::OnStarted, [&version, this]() { transition(version, Version::State::CHECKING_OUT); });
	...
	{
		qCritical() << "Cant check out in current state (state=" + toString(version.state()) + ")";
...
void VersionManager::transition(Version& version, Version::State state)
{
	...
	else
	{
		...
		qDebug() << "State transition: " << toString(old) << "=>" << toString(state);
...
{
	processes_[id] = proc;
	connect(proc, &Process::OnFinished, this, [this, id]() { this->unregisterProcess(id); });
}
