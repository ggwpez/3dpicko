...
static int start(int argc, char** argv)
{
	...
	APIController* api = new APIController(colony_detector, plate_detector, updater.mng(), db, &app);
	...
	setMessageHandler([&ws_server](QString msg) {
		if (ws_server)
			QMetaObject::invokeMethod(ws_server, "NewDebugLine", Q_ARG(QString, msg)); // send to other thread
	});
