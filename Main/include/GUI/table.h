...
template <typename Value> class Table : public JsonConvertable
{
  ...
  public:
	 ...
	 * @param json Value
	 */
	inline void addAsJson(Key const& key, QJsonObject const& json) { add(key, Marshalling::fromJson<Value>(json)); }

	/**
	 * @return Wether the key exist.
	 */
	inline bool exists(Key const& key) const { return (entries_.find(key) != entries_.end()); }
	...
	inline typename MapType::const_iterator begin() const { return entries_.cbegin(); }
	inline typename MapType::const_iterator end() const { return entries_.cend(); }

	inline typename MapType::const_iterator cbegin() const { return entries_.cbegin(); }
	inline typename MapType::const_iterator cend() const { return entries_.cend(); }
