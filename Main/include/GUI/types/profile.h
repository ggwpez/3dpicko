...
class Profile
{
  public:
	 ...
	 */
	static QJsonObject makeTextField(Profile::ID id, QString name, QString description, QString placeholder, QString defaultValue,
									 int min = 0, int max = 512);
	static QJsonObject makeNumberField(Profile::ID id, QString name, QString description, double min, double max, double step,
									   double placeholder, double defaultValue, QString unit);
	/**
	 * @brief Creates a field for a 2D point
	 * @return
	 */
	static QJsonObject makeVec2Field(Profile::ID id, QString name, QString description, Point min, Point max, double step,
									 Point placeholder, Point defaultValue, QString unit);
	/**
	 * @brief Creates a field for a 3D point
	 */
	static QJsonObject makeVec3Field(Profile::ID id, QString name, QString description, Point min, Point max, double step,
									 Point placeholder, Point defaultValue, QString unit);
	static QJsonObject makeVecField(Profile::ID id, QString name, int vec_size, QString description, Point min, Point max, double step,
									Point placeholder, Point defaultValue, QString unit);

	static QJsonObject makeCheckboxField(Profile::ID id, QString name, QString description, bool defaultValue);
	/**
	 * @brief Creates a radio box
	 * @param defaultValue default key used from options
	 */
	static QJsonObject makeEnumField(Profile::ID id, QString name, QString description, QVariantMap options, QString defaultValue);
