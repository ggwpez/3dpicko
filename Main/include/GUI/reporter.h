...
class Reporter
{
  public:
	 ...
	 */
	static Reporter fromDatabase(Database const& db, Report::ID id, Job::ID job, const std::map<Well, Colony::ID>& pick_positions,
  ...
  private:
	...
	Reporter(Report::ID id, Job const& job, QDateTime creation, const std::map<Well, Colony::ID>& pick_positions, const Image& image,
			 const DetectionResult* results_, QSet<Colony::ID> colonies_to_pick, Profile const& plate, Profile const& printer,
			 Profile const& socket, Profile const& octoprint, const std::vector<GcodeInstruction>& gcode);
	 ...
	 */
	QString createTable(QString title, QVector<QString> col_name, QVector<QVector<QString>> data) const;
