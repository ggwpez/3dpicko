...
namespace math
{
	...
	template <typename T> struct Range
	{
		 ...
		 */
		Range(const T& lower, const T& upper, bool lower_closed = true, bool upper_closed = true)
		 ...
		 */
		template <typename S = T> bool contains(S const& value) const
		{
		 ...
		 * Needs to have operators <, <=, > and <= for (S, T)
		 */
		template <typename S = T> inline bool excludes(S const& value) const { return !contains(value); }
	 ...
	 */
	template <typename T> inline T distance(T x1, T y1, T x2, T y2) { return std::sqrt(std::pow(x1 - x2, 2) + std::pow(y1 - y2, 2)); }

	// Strict include, points on the edge are considered outside
	template <std::size_t s1, std::size_t s2>
	inline bool polyIncludesPoly(std::array<cv::Point, s1> const& poly1, std::array<cv::Point, s2> const& poly2)
	{
	...
	inline bool polyIncludesPoly(std::vector<cv::Point> const& poly1, std::vector<cv::Point> const& poly2)
	{
		for (std::size_t i = 0; i < poly2.size(); ++i)
			if (cv::pointPolygonTest(poly1, poly2[i], false) <= 0)
				return false;
	 ...
	 */
	std::vector<Colony> filterByMinDistanceSlow(const std::vector<Colony>& colonies, const int r, const int d, const int min_dist);

	/**
	 * @brief Used for edge detection in the plate algorithm
	 * @return Raw components, non approximated
	 */
	std::vector<std::vector<cv::Point>> findConnectedComponentEdges(const cv::Mat& input, std::vector<std::vector<cv::Point>>& contours,
	 ...
	 */
	void drawText(cv::Mat& output, cv::Point pos, QString string, cv::Scalar color = cv::Scalar::all(255), double scale = 1,
				  int thickness = 1);
} // namespace math
} // namespace c3picko
#include <QMetaType>
