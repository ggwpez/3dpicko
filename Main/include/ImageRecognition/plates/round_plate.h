...
class RoundPlate : public Plate
{
  public:
	 ...
	 * rotation_matrix_
	 */
	RoundPlate(OuterBorder const& outer_border, InnerBorder const& innerBorder, Markers const& markers);
	RoundPlate(OuterBorder const& outer_border, InnerBorder const& innerBorder, Markers const& markers, std::size_t m1, cv::Point2d center,
  ...
  private:
	 ...
	 * @return [0,360]
	 */
	static double calculateRotation(Markers const& markers, std::size_t start_marker, cv::Point center);
