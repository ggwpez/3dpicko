...
class RectPlate : public Plate
{
  public:
	...
	RectPlate(OuterBorder const& outer_border, InnerBorder const& innerBorder, std::size_t a1, std::size_t h1, cv::Point2d inner_center,
			  cv::Point2d outer_center, cv::Point2d center, math::UnitValue center_error, double angle, cv::Rect aabb);
  ...
  private:
	 ...
	 * mirrored.
	 */
	static std::pair<std::size_t, std::size_t> findA1H1(const OuterBorder& outer_border, const InnerBorder& innerBorder);
	void									   findAndSetA1H1();
	/**
	 ...
	 */
	static double calculateRotation(const OuterBorder& cont, std::size_t a1, std::size_t h1);
