...
{
// TODO im not proud
RectPlate::RectPlate(const OuterBorder& outer_border, const InnerBorder& inner_border)
	: Plate((math::gravityCenter(outer_border) + math::gravityCenter(inner_border)) / 2,
			calculateRotation(outer_border, a1_ = findA1H1(outer_border, inner_border).first,
...
{
	center_error_ = std::abs(1 - outer_center_.x / inner_center_.x) + std::abs(1 - outer_center_.y / inner_center_.y);
}

RectPlate::RectPlate(const OuterBorder& outer_border, const InnerBorder& inner_border, std::size_t a1, std::size_t h1,
					 cv::Point2d inner_center, cv::Point2d outer_center, cv::Point2d center, math::UnitValue center_error, double angle,
					 cv::Rect aabb)
	: Plate(center, angle, aabb), outer_border_(outer_border), inner_border_(inner_border), a1_(a1), h1_(h1), inner_center_(inner_center),
	  outer_center_(outer_center), center_error_(center_error)
{
}

double RectPlate::calculateRotation(const OuterBorder& cont, std::size_t a1, std::size_t)
{
	...
	double ret = std::atan2((std::sin(v1) + std::sin(v2) + std::sin(v3) + std::sin(v4)) / 4,
							(std::cos(v1) + std::cos(v2) + std::cos(v3) + std::cos(v4)) / 4)
				 / M_PI * 180;
	return ret < 0 ? ret + 360 : ret;
}

std::pair<std::size_t, std::size_t> RectPlate::findA1H1(const OuterBorder& outer_border, const InnerBorder& inner_border)
{
	// index of the outer_border points
	std::size_t ia1 = 100, ih1 = 100;
	// the length of the two diagonal edges
	double ia1_min = std::numeric_limits<double>::max(), ih1_min = std::numeric_limits<double>::max();
	...
	double		a1_min = std::numeric_limits<double>::max(), h1_min = std::numeric_limits<double>::max();
...
void RectPlate::findAndSetA1H1() { std::tie(a1_, h1_) = findA1H1(outer_border_, inner_border_); }
...
RectPlate* RectPlate::rotated() const
{
	...
	return new RectPlate(new_outer_border, new_inner_border, a1_, h1_, inner_center_ - left_top, outer_center_ - left_top,
...
void RectPlate::mask(const cv::Mat& in, cv::Mat& out) const
{
	...
	cv::drawContours(mask, std::vector<std::vector<cv::Point>>{{inner_border_.begin(), inner_border_.end()}}, 0, cv::Scalar::all(255), -1);

	out = in & mask;
}

const RectPlate::InnerBorder& RectPlate::innerBorder() const { return inner_border_; }

const RectPlate::OuterBorder& RectPlate::outerBorder() const { return outer_border_; }

template <> QJsonObject Marshalling::toJson(RectPlate* const& value)
{
	QJsonObject obj;
...
template <> RectPlate* Marshalling::fromJson(const QJsonObject& obj)
{
