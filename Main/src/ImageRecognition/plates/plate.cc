...
Plate::Plate(cv::Point2d center, double angle, cv::Rect bb)
	: center_(center), angle_(angle), aabb_(bb), rotation_matrix_(cv::getRotationMatrix2D(center_, angle_, 1))
{
}

void Plate::crop(const Plate& original, const Plate& rotated, const cv::Mat& in, cv::Mat& out)
{
	cv::warpAffine(in, out, original.rotationMatrix(), cv::Size(in.cols, in.rows));
...
template <> QJsonObject Marshalling::toJson(std::unique_ptr<Plate> const& value) { return Marshalling::toJson(value.get()); }

template <> std::unique_ptr<Plate> Marshalling::fromJson(const QJsonObject& obj)
{
	return std::unique_ptr<Plate>(Marshalling::fromJson<Plate*>(obj));
}

template <> QJsonObject Marshalling::toJson(Plate* const& value)
{
	QJsonObject obj;
	RoundPlate* round;
	RectPlate*  rect;
...
template <> Plate* Marshalling::fromJson(const QJsonObject& obj)
{
	if (obj["type"] == "round")
