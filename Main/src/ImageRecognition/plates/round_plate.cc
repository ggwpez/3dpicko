...
namespace c3picko
{
RoundPlate::RoundPlate(const RoundPlate::OuterBorder& outer_border, const RoundPlate::InnerBorder& inner_border,
					   const RoundPlate::Markers& markers)
	// TODO gravityCenter called twice...
	: Plate(math::gravityCenter(inner_border), calculateRotation(markers, findM1(markers), math::gravityCenter(inner_border)),
			cv::boundingRect(outer_border)),
	  outer_border_(outer_border), inner_border_(inner_border), markers_(markers), m1_(findM1(markers))
{
}

RoundPlate::RoundPlate(const RoundPlate::OuterBorder& outer_border, const RoundPlate::InnerBorder& inner_border,
					   const RoundPlate::Markers& markers, std::size_t m1, cv::Point2d center, double angle, cv::Rect aabb)
	: Plate(center, angle, aabb), outer_border_(outer_border), inner_border_(inner_border), markers_(markers), m1_(m1)
{
}

std::size_t RoundPlate::findM1(const RoundPlate::Markers& markers)
{
	double d01 = cv::norm(markers[0] - markers[1]), d12 = cv::norm(markers[1] - markers[2]), d20 = cv::norm(markers[2] - markers[0]);
...
double RoundPlate::calculateRotation(RoundPlate::Markers const& markers, std::size_t start_marker, cv::Point center)
{
	...
	double ret = std::atan2((std::sin(a1) + std::sin(a2) + std::sin(a3)) / 3, (std::cos(a1) + std::cos(a2) + std::cos(a3)) / 3);
...
const RoundPlate::InnerBorder& RoundPlate::innerBorder() const { return inner_border_; }

const RoundPlate::InnerBorder& RoundPlate::outerBorder() const { return outer_border_; }
...
RoundPlate* RoundPlate::rotated() const
{
	...
	return new RoundPlate(new_outer_border, new_inner_border, new_markers, m1_, center() - left_top, 0, aabb);
}

void RoundPlate::mask(const cv::Mat& in, cv::Mat& out) const
{
	cv::Mat mask(in.rows, in.cols, in.type(), cv::Scalar());
	cv::drawContours(mask, std::vector<std::vector<cv::Point>>{inner_border_}, 0, cv::Scalar::all(255), -1);

	out = in & mask;
}

template <> QJsonObject Marshalling::toJson(RoundPlate* const& value)
{
	QJsonObject obj;
...
template <> RoundPlate* Marshalling::fromJson(const QJsonObject& obj)
{
	RoundPlate::InnerBorder inner;
	RoundPlate::OuterBorder outer;
	RoundPlate::Markers		markers;
	QJsonArray				jinner(obj["inner"].toArray()), jouter(obj["outer"].toArray()), jmarkers(obj["markers"].toArray());
