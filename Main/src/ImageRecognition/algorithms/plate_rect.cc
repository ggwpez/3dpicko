...
namespace algorithms
{
	PlateRect::PlateRect()
		: Algorithm("kRECT", "Rect1", "Detects a plate with red frame attached",
					{(AlgoStep)Colonies1::cvt, (AlgoStep)PlateRect::threshold, (AlgoStep)PlateRect::detect}, {}, true, 5000)
	{
	}
	...
	void PlateRect::threshold(AlgorithmJob*, PlateResult* result)
	{
		...
		cv::Mat tmp, _;
		double  otsu = cv::threshold(input, _, 0, 255, CV_THRESH_BINARY | CV_THRESH_OTSU);
	...
	void PlateRect::detect(AlgorithmJob* base, PlateResult* result)
	{
		...
		math::Range<int>					area((original.rows * original.cols) / 32, std::numeric_limits<int>::max());
		cv::Mat&							output(result->newMat(original));
		...
		for (auto it = outer_edges.begin(); it != outer_edges.end();)
		{
			...
			double w = math::distance(polygon[0].x, polygon[0].y, polygon[1].x, polygon[1].y);
		...
		std::vector<std::tuple<std::size_t /*inner*/, std::size_t /*outer*/, double>> corner_distances;
		for (std::size_t i = 0; i < inner_edges.size(); ++i)
		{
			...
			for (std::size_t j = 0; j < outer_edges.size(); ++j)
			{
				...
				if (!math::polyIncludesPoly(outer, inner))
					continue;
		...
		auto it = std::min_element(corner_distances.begin(), corner_distances.end(),
								   [](std::tuple<std::size_t, std::size_t, double> const& a,
									  std::tuple<std::size_t, std::size_t, double> const& b) { return std::get<2>(a) < std::get<2>(b); });
		...
		// Create the plates
		std::unique_ptr<RectPlate> unrotated(std::make_unique<RectPlate>(outer_edge, inner_edge));
