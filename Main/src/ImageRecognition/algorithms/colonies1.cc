...
void Colonies1::label(AlgorithmJob* base, DetectionResult* result) {
  ...
  std::function<void(int, int)> map =
	  [&centers, &colonies, &labeled, &stats, base, &input, &show_excluded,
	   ...
	   &_convexity](int start, int off) -> void {
	Colony::ID id = start;
	const double scale_x = 1.d / input.cols, scale_y = 1.d / input.rows;

	for (int o = 0; o < off; ++o) {
	  int colony_index = start + o;
	  ...
	  if (!_area.contains(area)) {
		colonies[colony_index] =
			Colony(center.x * scale_x, center.y * scale_y, area, 0,
				   std::sqrt(area * M_1_PI), 0, id++, "area");
		continue;
	  }
	  // Filter by roundness
	  else {
		...
		if (_aabb_ratio.excludes(std::min(w, h) / double(std::max(w, h)))) {
		  colonies[colony_index] =
			  Colony(center.x * scale_x, center.y * scale_y, area, 0,
					 std::sqrt(area * M_1_PI), 0, id++, "aabb_ratio");
		}
		...
		if (!excluded_by.isEmpty())
		  colonies[colony_index] =
			  Colony(center.x * scale_x, center.y * scale_y, area, 0,
					 std::sqrt(area * M_1_PI), 0, id++, excluded_by);
		else {
...
contours[0], *reinterpret_cast<cv::Mat*>(base->input().front()));*/
		  colonies[colony_index] =
			  Colony(center.x * scale_x, center.y * scale_y, area, circ,
					 circ * M_1_PI_2, br, id++, "");
		}
	  }
	}
  ...
  // Parallel part
  const int tasks = 4;
  const int task_size = centers.rows / tasks;
  Q_ASSERT(tasks && task_size);
  QFutureSynchronizer<void> barrier;  // synchronises in dtor
  for (int i = 0; i < tasks; ++i) {
	auto future =
		QtConcurrent::run(base->pool(), map, task_size * i, task_size);
	barrier.addFuture(future);
  }

  // NOTE QtConcurrent::blockingMappedReduced could also be used, but has the
  // disadvantage that a ThreadPool cant be specified .
