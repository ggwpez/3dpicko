...
namespace algorithms
{
	PlateRound::PlateRound()
		: Algorithm("kROUND", "Round1", "Detects a round plate with red frame attached",
					{(AlgoStep)Colonies1::cvt, (AlgoStep)PlateRect::threshold, (AlgoStep)PlateRound::detect}, {}, true, 5000)
	{
	}
	...
	void PlateRound::detect(AlgorithmJob* base, PlateResult* result)
	{
		...
		cv::Mat& output = result->newMat(*static_cast<cv::Mat*>(base->input().front()));
		...
		if (edges.size() != 2)
			throw Exception("Did not detect exactly two contours");
		...
		{
			double inner_err
				= qAbs(cv::contourArea(edges[inner]) - (M_PI * inner_e.size.width * inner_e.size.height)) / cv::contourArea(edges[inner]);
			double outer_err
				= qAbs(cv::contourArea(edges[outer]) - (M_PI * outer_e.size.width * outer_e.size.height)) / cv::contourArea(edges[outer]);

			if (inner_err >= outer_err)
				throw Exception("Outer edge was rounder than inner edge");
		}

		double inner_r  = (inner_e.size.width + inner_e.size.height) / 4; // /4 b its the diameter
		...
		std::array<cv::Point, 3> markers;
		{
			...
			for (std::size_t i = 0; i < raw_edges[outer].size(); ++i)
			{
				...
				if (d > outer_r * margin_r)
					outer_points.push_back(raw_edges[outer][i]);
			}

			cv::kmeans(outer_points, 3, labels, cv::TermCriteria(cv::TermCriteria::COUNT, 1, 10), 1, cv::KMEANS_PP_CENTERS);
		...
		// Create the plates
		std::unique_ptr<RoundPlate> unrotated(std::make_unique<RoundPlate>(edges[outer], edges[inner], markers));
		...
		// Draw the inner ellipse and markers (optional)
		{
			...
			cv::drawMarker(output, markers[unrotated->m1()], cv::Scalar(0, 255, 0), cv::MARKER_CROSS, 30, 2);
			cv::drawMarker(output, markers[(unrotated->m1() + 1) % 3], cv::Scalar(255, 255, 0), cv::MARKER_TILTED_CROSS, 30, 2);
			cv::drawMarker(output, markers[(unrotated->m1() + 2) % 3], cv::Scalar(255, 255, 0), cv::MARKER_TILTED_CROSS, 30, 2);
		}
