...
static int start(int argc, char** argv)
{
	...
	AlgorithmManager* colony_detector = new AlgorithmManager(algo_pool, {new Colonies1(), new Fluro1()}, &app);
	AlgorithmManager* plate_detector  = new AlgorithmManager(algo_pool, {new PlateRect(), new PlateRound()}, &app);
	...
	QObject::connect(db, &Database::OnDataChanged, autos, &Autosaver::dataChanged);
	QObject::connect(autos, &Autosaver::OnAutosaveNeeded, db, &Database::autosave);
	QObject::connect(autos, &Autosaver::OnAutosaveSkipp, db, &Database::autosaveSkipped);
	autos->start();
	...
	APIController* api = new APIController(colony_detector, plate_detector, nullptr, db, &app);

	// Static file controller
	settings.beginGroup("files");
	StaticFileController* staticFileController = new StaticFileController(settings, paths::docRoot().toSystemAbsolute(), &app);
	settings.endGroup();
	...
	// Calls stopLog when something throws an exception als QCoreApplication::abouToQuit does not get emitted
	DtorCallback guard(
		[db] {
			db->saveToFile();
			stopLog();
	...
	QObject::connect(Logger::instance(), &Logger::OnNewLine, ws_server, &WsServer::NewDebugLine);
