...
Process::Process(QString binary, QStringList args, QObject* _parent)
	: QObject(_parent), binary_(binary), git_(nullptr), args_(args), exec_dir_(ResourcePath::fromServerRelative(""))
{
}

Process::Process(QString binary, QStringList args, const ResourcePath& exec_dir, QObject* _parent)
	: QObject(_parent), binary_(binary), git_(nullptr), args_(args), exec_dir_(exec_dir)
{
}
...
Process* Process::gitClone(QString repo, ResourcePath const& into, QStringList arguments)
{
...
Process* Process::gitCheckout(QString branch, const ResourcePath& repo) { return new Process("git", {"checkout", branch}, repo); }
...
Process* Process::make(const ResourcePath& build, QStringList targets, int cores)
{
...
{
	if (git_)
		throw Exception("Cant start process twice");
	...
	if (!git_->waitForStarted(3000))
		emit OnFailure("Could not start (path=" + git_path + "): " + git_->errorString());
	emit OnStarted();
	if (!git_->waitForFinished(getSubprocessTimeoutMs()))
		emit OnFailure("Timed out (path=" + git_path + ", max_time=" + QString::number(getSubprocessTimeoutMs()) + "ms)");
	else if (git_->exitStatus() != QProcess::NormalExit)
		emit OnFailure("Crashed (path=" + git_path + "): " + git_->readAllStandardError());
	else if (git_->exitCode() != 0)
		emit OnFailure("Error (path=" + git_path + "): " + git_->readAllStandardError());
	else
	{
		QString output = git_->readAllStandardOutput().replace("\"", ""); // FIXME why is this needed?
