...
namespace c3picko
{
Image::Image(Image::ID id, QString original_name, QString description, ResourcePath path, QDateTime uploaded, int width, int height)
...
{
	if (image_.empty() || !path_.isEmpty()) // path_ should be empty, otherwise we may create two
	...
	if (!file.open())
	{
		qCritical() << "Could not write image to drive:" << file.errorString() << "Directory:" << paths::uploadFolder().toSystemAbsolute();
		return false;
	}
	else
	{
...
bool Image::readData(QByteArray& output) const
{
	...
	{
		qWarning() << "Could not open file:" << path_.toSystemAbsolute() << ":" << file.errorString();
		return false;
	}
...
bool Image::decodeCvMat(QByteArray data, cv::Mat& output)
{
	...
	output = cv::imdecode(cv::Mat(1, data.size(), CV_8UC1, data.data()), cv::IMREAD_COLOR);
	return (!output.empty());
}
...
template <> Image Marshalling::fromJson(const QJsonObject& obj)
{
	return Image(Marshalling::fromJson<QString>(obj["id"]), Marshalling::fromJson<QString>(obj["original_name"]),
				 Marshalling::fromJson<QString>(obj["description"]), ResourcePath::fromDocRootAbsolute(obj["path"].toString()),
				 Marshalling::fromJson<QDateTime>(obj["uploaded"].toObject()), obj["width"].toInt(), obj["height"].toInt());
}
} // namespace c3picko
