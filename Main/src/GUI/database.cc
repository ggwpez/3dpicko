...
Database::Database(const QSettings& settings, QObject* parent)
	: QObject(parent), file_path_(paths::root() + "/database.json"), read_only_(!settings.value("saveChanges", true).toBool()),
	  job_id_(200), profile_id_(300)
{
	...
	if (file.open(QIODevice::ReadOnly))
	{
		...
		throw Exception("Error reading database from file: " + file_path_.toSystemAbsolute() + " (ignoreEmpty=false)");
}
...
void Database::saveToFile()
{
	try
	{
		...
		if (file.open(QIODevice::WriteOnly))
		{
			...
			qCritical() << "Error saving database to file:" << file_path_.toSystemAbsolute();
	}
	catch (...)
	{
		qCritical() << "Error saving database to file:" << file_path_.toSystemAbsolute();
...
void Database::autosaveSkipped() { qDebug() << "Skipped Autosave (no changes)"; }
...
const Database::JobTable& Database::deletedJobs() const { return deleted_jobs_; }
...
const Database::ImageTable& Database::deletedImages() const { return deleted_images_; }
...
const Database::VersionIdVector& Database::installedVersions() const { return versions_installed_; }

Database::PlateTable& Database::detectedPlates() { return detected_plates_; }

const Database::PlateTable& Database::detectedPlates() const { return detected_plates_; }
...
void Database::read(QJsonObject const& obj)
{
	...
	default_octoprint_ = Marshalling::fromJson<Profile::ID>(obj["default_octoprint"]);
...
void Database::write(QJsonObject& obj) const
{
	...
	QJsonArray arr;
	for (auto e : versions_installed_)
		arr.push_back(e);
...
bool Database::checkIntegrity() const
{
	...
	for (auto const& pair : jobs_)
	{
		...
		if (!profiles_.exists(job.plate()) || !profiles_.exists(job.printer()) || !profiles_.exists(job.socket())
			|| !profiles_.exists(job.octoprint()))
