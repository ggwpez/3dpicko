...
namespace c3picko
{
APIController::APIController(AlgorithmManager* colony_detector, AlgorithmManager* plate_detector, VersionManager* version_manager,
							 Database* db, QObject* parent)
	: QObject(parent), colony_detector_(colony_detector), plate_detector_(plate_detector), version_manager_(version_manager), db_(db),
	  input_(new APIInput(this)), output_(new APIOutput(this))
{
	...
	if (version_manager)
	{
		...
		connect(version_manager, &VersionManager::OnSwitched, this, &APIController::OnVersionSwitched);
	}
}
...
QJsonObject APIController::createVersionList() const
{
	...
	for (Version::ID id : db_->installedVersions())
...
void APIController::DeleteImage(Image::ID id, QObject* client)
{
	...
	if (image.deleteFile())
	{
		...
		db_->images().remove(id); // Carefull here, if we use a reference to image instead, it will

		if (db_->detectedPlates().exists(id))
			db_->detectedPlates().remove(id);
...
void APIController::UploadImage(Image image, QObject* client)
{
	...
	std::shared_ptr<PlateResult> result(std::make_shared<PlateResult>(db_->newResultId()));

	AlgorithmJob* ajob = plate_detector_->createJob(aid, jid, result, QJsonObject());
	ajob->pushInput(raw.get());

	if (!ajob)
		emit OnImageCreateError("Internal Error: Algorithm not found (" + aid + ")", client);
	else
	{
		...
		// existing
		connect(ajob, &AlgorithmJob::OnAlgoSucceeded, ajob, [this, client, image, ajob, result] {
			if (!result->rotatedImage())
				return emit OnImageCreateError("Assertion failure: Result from PlateDetection was null", client);

			Image cropped(*result->rotatedImage(), image.originalName(), image.description(), image.uploaded());
			...
			else
			{
				...
				db_->detectedPlates().add(cropped.id(), std::move(result->rotated()));
			}

			emit this->OnImageCreated(cropped.id(), client);
			qDebug() << "Detected Plate in" << ajob->tookMs() << "ms";
		});
		connect(ajob, &AlgorithmJob::OnAlgoFailed, ajob,
				[this, ajob, client] { emit OnImageCreateError(ajob->result()->stageError(), client); });
...
void APIController::createSettingsProfile(Profile& profile_wo_id, QObject* client)
{
	...
	if (profile_wo_id.type() == ProfileType::OCTOPRINT)
	{
		for (auto it = db_->profiles().begin(); it != db_->profiles().end(); ++it)
			if (it->second.type() == ProfileType::OCTOPRINT)
				return emit OnProfileCreateError("Cant create more than one octoprint profile", client);
	}
...
void APIController::setDefaultSettingsProfile(Profile::ID id, QObject* client)
{
	...
	else
	{
		...
		{
			qCritical() << "Database corrupt or wrong version: Profile" << id << "had unknown type" << (int)profile.type();
...
void APIController::setStartingWell(Job::ID id, Profile::ID plate_id, int row, int col, QObject* client)
{
	if (!db_->jobs().exists(id))
	{
	...
	else
	{
		...
		if (qBound(1, row, plate->numberOfRows()) != row)
			emit OnSetStartingWellError("Invalid starting row " + QString::number(row) + " for plate profile: " + plate_id, client);
		else if (qBound(1, col, plate->numberOfColumns()) != col)
			emit OnSetStartingWellError("Invalid starting col " + QString::number(row) + " for plate profile: " + plate_id, client);
		else
		{
			Job& job = db_->jobs().get(id);
...
void APIController::setColoniesToPick(Job::ID id, QSet<Colony::ID> ex_user, QSet<Colony::ID> in_user, quint32 number, QObject* client)
{
	if (!db_->jobs().exists(id))
	...
	if (!job.resultJob() || !job.resultJob()->result())
		return emit OnSetColoniesToPickError("Job " + id + " has no detected colonies attatched to it", client);

	DetectionResult* result = static_cast<DetectionResult*>(job.resultJob()->result().get());
	...
	// Input validation
	{
		if (!(ex_user & ex_algo).empty())
			return emit OnSetColoniesToPickError("Cant exclude colonies that are already excluded by the "
												 "algorithm",
												 client);
		if (!(ex_user - in_algo).empty())
			return emit OnSetColoniesToPickError("Cant exclude colonies that are not included by the algorithm", client);
		if (!(in_user & in_algo).empty())
			return emit OnSetColoniesToPickError("Cant include colonies that are already included by the "
												 "algorithm",
												 client);
		if (!(in_user - ex_algo).empty())
			return emit OnSetColoniesToPickError("Cant include colonies that are not excluded by the algorithm", client);
		if (!(in_user & ex_user).empty())
			return emit OnSetColoniesToPickError("Cant include and exclude a colony at the same time", client);
	}
	...
	if (number > all)
		return emit OnSetColoniesToPickError("Cant pick more colonies than available", client);
...
std::shared_ptr<AlgorithmJob> APIController::detectColonies(Job::ID job_id, QString algo_id, QJsonObject settings, QObject* client)
{
	if (!db_->jobs().exists(job_id))
	{
		qWarning() << "Client requested colony detection for unknown job, id=" << job_id;
	...
	else
	{
		...
		else
		{
			...
			{
				qCritical() << "CV could not read image" << img.path().toSystemAbsolute();
			...
			else
			{
				...
				std::shared_ptr<DetectionResult> result(std::make_shared<DetectionResult>(result_id));
				std::shared_ptr<AlgorithmJob>	algo_job(colony_detector_->createJob(algo_id, algo_job_id, result, settings));
				AlgorithmJob*					 raw = algo_job.get();

				algo_job->pushInput(image.get());
				if (db_->detectedPlates().exists(img_id))
					// Nobody can delete an image while the job uses it, so the plate also wont be deleted
				...
				else
				{
					...
					connect(raw, &AlgorithmJob::OnAlgoSucceeded, raw, [this, client, job_id, raw, result] {
						emit this->OnColonyDetected(job_id, &result->colonies(), client);
						qDebug() << "Detected" << result->colonies().size() << "in" << raw->tookMs() << "ms";
					});
					connect(raw, &AlgorithmJob::OnAlgoFailed, raw,
							[this, raw, client] { emit OnColonyDetectionError(raw->result()->stageError(), client); });
...
void APIController::updateDetectionSettings(Job::ID job_id, QString algo_id, QJsonObject settings, QObject* client)
{
	std::shared_ptr<AlgorithmJob> job = detectColonies(job_id, algo_id, settings, client);

	if (job)
		job->start(true, false);
}
...
void APIController::startJob(Job::ID id, QObject* client)
{
	...
	if (!job.resultJob() || !job.resultJob()->result())
		return emit OnJobStartError("Job " + id + " could not find its detection results: " + job.resultID(), client);
	DetectionResult* result = static_cast<DetectionResult*>(job.resultJob()->result().get());

	if (!db_->profiles().exists(job.printer()) || !db_->profiles().exists(job.socket()) || !db_->profiles().exists(job.plate())
		|| !db_->profiles().exists(job.octoprint()))
		return emit OnJobStartError("Internal error: Cant find printer, socket, plate or octoprint profile", client);
	...
	// Convert the colony coordinates to real world coordinates
	{
		...
		{
			auto colony = std::find_if(result->includedBegin(), result->includedEnd(), [&it](Colony const& c) { return c.id() == *it; });

			if (colony == result->includedEnd())
				return emit OnJobStartError("Internal error: Cant find selected colony", client);

			// Invert the y-axis. FIXME get the frame size from the plate profile
			// coords.push_back(Point(colony->x() * 128, (1.0 - colony->y()) * 85.9));
			coords.push_back(Point(colony->x() * 98 + 12.75, // 11.5 is the offset of the plate
	...
	std::vector<GcodeInstruction> code = gen.CreateGcodeForTheEntirePickingProcess(job.startingRow(), job.startingCol(), coords);

	Reporter reporter(Reporter::fromDatabase(*db_, db_->newReportId(), job.id(), pick_positions, code));
	...
	connect(cmd, &Command::OnStatusErr, [](QJsonValue status) { qWarning() << status.toString(); });
	connect(cmd, &Command::OnNetworkErr, [](QString error) { qWarning() << error; });
...
QJsonObject APIController::createImageList() const
{
	...
	for (auto const& pair : db_->images())
		pairs.push_back(std::cref<Image>(pair.second)); // Copy here bc reference_wrapper does not have move ctor ?!
	std::sort(pairs.begin(), pairs.end(), [](std::reference_wrapper<Image const> a, std::reference_wrapper<Image const> b) {
...
QJsonObject APIController::createDeleteImage(Image const& img) { return {{"id", img.id()}}; }

QJsonObject APIController::createDeleteJob(Job const& job) { return {{"id", job.id()}}; }
...
bool APIController::isProfileUsedByJob(Profile::ID profile)
{
	for (auto const& pair : db_->jobs())
	{
		...
		if (job.plate() == profile || job.printer() == profile || job.socket() == profile || job.octoprint() == profile)
			return true;
	}
...
{
	return (db_->defaultPlate() == profile || db_->defaultSocket() == profile || db_->defaultPrinter() == profile
			|| db_->defaultOctoconfig() == profile);
}

void APIController::request(QJsonObject request, QString raw_request, QObject* client)
{
	try
	{
