...
namespace c3picko
{
Report::Report(Report::ID id, const Job& job, QDateTime creation, const std::map<Colony::ID, Well>& pick_positions, Image const& image,
			   const DetectionResult& result, QSet<Colony::ID> colonies_to_pick, const Profile& plate_, const Profile& printer,
	...
	: id_(id), job_(job), creation_(creation), pick_positions_(pick_positions), image_(image), result_(result),
	  colonies_to_pick_(colonies_to_pick), plate_(plate_), printer_(printer), socket_(socket), octoprint_(octoprint)
{
	if (!job.resultJob())
		throw Exception("Job cant have empty result");
}

Report Report::fromDatabase(Database& db, ID id, Job::ID jid, const std::map<Colony::ID, Well>& pick_positions)
{
	...
	return Report(id, job, QDateTime::currentDateTime(), pick_positions, img, result, job.coloniesToPick(), plate, printer, socket,
				  octoprint);
}
...
void Report::writePdfReport(QPdfWriter* pdf)
{
	...
	pdf->setTitle("Report #" + id_ + " for job #" + job_.id() + " (" + job_.name().left(20) + ")");
	...
	cursor.insertHtml("<br><br>" + image_.uploaded().toString(dateTimeFormat()) + ": " + "Image #" + image_.id().left(10)
					  + "... created<br>" + job_.created().toString(dateTimeFormat()) + ": " + "Job #" + job_.id() + " created<br>"
					  + algo->start().toString(dateTimeFormat()) + ": " + "AlgorithmJob #" + algo->id() + " created<br>"
					  + algo->end().toString(dateTimeFormat()) + ": " + "DetectionResult #" + result_.id() + " created<br><br>");
	/*"<center>Job "
  + job_.id() + " will pick " + QString::number(pick_positions_.size()) + " colonies from all "
  + QString::number(colonies.size()) + " detected</center><br><br><br>");*/
	...
	static char const* headers[columns] = {" Colony ", " Picked ", " Well ", " Excluded by ", " Area [pxl] "};
	...
	for (std::size_t i = 0; i < rows; i++)
	{
		...
		QString data[columns] = {QString::number(c.id()), (picked ? "YES" : "NO"), well, comment, QString::number(c.area(), 'e', 2)};
