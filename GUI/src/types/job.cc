...
namespace c3picko
{
Job::Job(ID id, Image::ID img_id, QString name, QString description, QDateTime job_created, QStack<AlgorithmResult::ID> results,
		 Profile::ID printer, Profile::ID socket, int starting_row, int starting_col, int step)
	: id_(id), img_id_(img_id), name_(name), description_(description), created_(job_created), result_ids_(results), printer_(printer),
	  socket_(socket), starting_row_(starting_row), starting_col_(starting_col), step_(step)
{
}
...
void Job::setResultAlgorithmJob(std::shared_ptr<AlgorithmJob> job) { result_job_ = job; }

void Job::setResultID(const AlgorithmResult::ID& result_id) { result_ids_.push(result_id); }
...
void Job::setcoloniesToPick(const QSet<Colony::ID>& colonies_to_pick) { colonies_to_pick_ = colonies_to_pick; }
...
template <> Job Marshalling::fromJson(const QJsonObject& obj)
{
	...
	return Job(Marshalling::fromJson<QString>(obj["id"]), Marshalling::fromJson<QString>(obj["img_id"]),
			   ...
			   Marshalling::fromJson<QDateTime>(obj["created"].toObject()), detections, Marshalling::fromJson<QString>(obj["printer"]),
			   Marshalling::fromJson<QString>(obj["socket"]), obj["starting_row"].toInt(), obj["starting_col"].toInt(),
			   obj["step"].toInt());
}
} // namespace c3picko
