...
void APIInput::serviceRequest(QJsonObject& request, QObject* client)
{
	...
	else if (path == "deleteimage")
	{
		...
		if (image.deleteFile())
		{
			...
			api->db().images().remove(id); // Carefull here, if we use a reference to image instead, it will
	...
	else if (path == "createsettingsprofile")
	{
		...
		emit api->OnProfileCreated(profile, client);
	}
	else if (path == "updatesettingsprofile")
	{
		...
		else
		{
			...
			emit api->OnProfileUpdated(profile, client);
		}
	}
	else if (path == "deletesettingsprofile")
	{
		...
		else
		{
			...
			emit api->OnProfileDeleted(id, client);
	...
	else if (path == "getpositions")
	{
		...
		else
		{
			...
			// Set the connection context to detector, so in case it timeouts it
			// dosent leak memory
			connect(detector, &ColonyDetector::OnFinished, client, [this, client, detector](qint64 ms) {
				qDebug() << "Colony detected in" << ms << "ms";
				emit api->OnColonyDetected(detector, client);
			});
			detector->setAutoDelete(false);
			api->pool()->start(detector);

			/*std::vector<cv::Vec3f> coords = ColonyDetector::colonyDetection(data,
			""); coords = Conversion::createColonyCoordinates(coords,
			data.cols,data.rows); QJsonArray json_coords; qDebug() << coords.size() <<
			"Colonies"; for (int i = 0; i < coords.size(); ++i)
			{
					json_coords.push_back(QJsonArray({coords[i][0], coords[i][1],
			coords[i][2]}));
			}
