...
void APIInput::serviceRequest(QJsonObject& request, QString const& raw_request, QObject* client)
{
	...
	else if (path == APICommands::START_JOB)
	{
		...
		Profile::ID octoprint = Marshalling::fromJson<Profile::ID>(req_data["octoprint_profile"]);
	...
	{
		// Get image data
		QByteArray img_data(QByteArray::fromBase64(Marshalling::fromJson<QString>(req_data["file"]).toUtf8())); // TODO ugly code
	...
	else if (path == APICommands::SET_COLONIES_TO_PICK)
	{
		...
		QSet<Colony::ID> ex_user = Marshalling::fromJson<QSet<Colony::ID>>(req_data["ex_user"]),
						 in_user = Marshalling::fromJson<QSet<Colony::ID>>(req_data["in_user"]);
