...
QJsonObject APIInput::createDeleteImage(Image img) {
  return {{"id", img.id()}};
}
...
void APIInput::service(QJsonObject &request, QObject *client) {
  ...
  if (path == "getimagelist") {
    ...
    qDebug("%s", QString::fromUtf8(QJsonDocument(req_data).toJson())
                     .toLatin1()
                     .constData());
    ...
    if (!api->db().images().exists(img_id)) {
      ...
      ColonyDetector *detector = new ColonyDetector(
          data.clone()); // Clone to avoid thread problems TODO debug
      // Set the connection context to detector, so in case it timeouts it
      // dosent leak memory
      connect(detector, &ColonyDetector::OnFinished, detector,
              [this](qint64 ms) {
    ...
    PrinterProfile *printerp =
        (PrinterProfile *)(api->db().profiles().get("302"));
    PlateSocketProfile *socket =
        (PlateSocketProfile *)(api->db().profiles().get("303"));
