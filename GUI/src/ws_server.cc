...
void WsServer::NewConnection()
{
	...
	connect(pSocket, SIGNAL(error(QAbstractSocket::SocketError)), this, SLOT(clientError(QAbstractSocket::SocketError)));
...
void WsServer::clientError(QAbstractSocket::SocketError ec) { qDebug() << "WsServer::clientError" << ec; }
...
bool WsServer::StartListen()
{
	if (server_->listen(QHostAddress(host_), port_))
	{
		...
		connect(server_, &QWebSocketServer::acceptError, this, &WsServer::acceptError);
		connect(server_, &QWebSocketServer::peerVerifyError, this, &WsServer::peerVerifyError);
		connect(server_, &QWebSocketServer::sslErrors, this, &WsServer::sslErrors);
		connect(server_, &QWebSocketServer::serverError, this, &WsServer::serverError);
...
void WsServer::acceptError(QAbstractSocket::SocketError ec) { qWarning() << "WsServer::acceptError " << ec; }

void WsServer::peerVerifyError(const QSslError& error) { qWarning() << "WsServer::peerVerifyError " << error; }

void WsServer::serverError(QWebSocketProtocol::CloseCode ec) { qWarning() << "WsServer::serverError " << ec; }
