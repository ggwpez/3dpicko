...
namespace c3picko
{
APIController::APIController(AlgorithmPipeline* detector, Database* db, QObject* parent)
...
void APIController::getPositions(Image::ID id, void* algorithm, void* algo_settings, QObject* client)
{
	if (!db_->images().exists(id))
	{
	...
	{
		Image&  img = db_->images().get(id); // Non const& bc readCvMat() can set the cache
		cv::Mat data;
