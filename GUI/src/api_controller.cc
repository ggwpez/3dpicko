	...
	else
	{
		...
		else
		{
			connect(ajob, &AlgorithmJob::OnAlgoSucceeded, client, [this, client, image, ajob, result] {
				Image cropped(*result->stack().back(), image.originalName(), image.description(), image.uploaded());
...
void APIController::cropImage(Image::ID id, QObject* client)
{
	...
	else
	{
		...
		// just autodetect the frame
		{
			...
			AlgorithmJob*	algo_job	  = plate_detector_->createJob(mat, "1", result_job_id, result, QJsonObject());

			if (!algo_job)
			{
				emit OnImageCropError("Internal error, could not create Plate detection job", client);
				return;
			}
			else
			{
				...
				connect(algo_job, &AlgorithmJob::OnAlgoSucceeded, client, [this, client, algo_job, result, new_name] {
					cv::Mat cropped_mat(result->oldMat());
					Image   cropped = Image(cropped_mat, new_name,
				...
				connect(algo_job, &AlgorithmJob::OnAlgoFailed, client,
						[this, result, client] { emit this->OnImageCropError(result->stageError(), client); });
...
void APIController::setColoniesToPick(Job::ID id, quint32 number, QObject* client)
{
	if (!db_->jobs().exists(id))
	{
	...
	else
	{
		...
		{
			emit OnSetColoniesToPickError("Job " + id + " has no detected colonies attatched to it", client);
		}
		else
		{
			...
			{
				emit OnSetColoniesToPickError("Cant pick more colonies than available", client);
				return;
			}
...
AlgorithmJob* APIController::detectColonies(Job::ID job_id, QString algo_id, QJsonObject settings, QObject* client)
{
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				AlgorithmJob* algo_job = colony_detector_->createJob(image, algo_id, result_job_id, result, settings);
				...
				{
					connect(algo_job, &AlgorithmJob::OnAlgoSucceeded, client, [this, client, algo_job, result] {
...
void APIController::startJob(Job::ID id, QObject* client)
{
	...
	// FIXME check
	PrinterProfile*		printerp = (PrinterProfile*)(db_->profiles().get(job.printer()));
	PlateSocketProfile* socket   = (PlateSocketProfile*)(db_->profiles().get(job.socket()));
	...
	for (auto it = good_colonies.begin(); it != good_colonies.end(); ++it)
		coords.push_back(Point(all_colonies[*it].x() * 128, (1.0 - all_colonies[*it].y()) * 85.9));
	...
	if (!file.open(QIODevice::WriteOnly))
		throw Exception("Could not save gcode");
	QTextStream ts(&file);
