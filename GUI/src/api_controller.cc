...
void APIController::getPositions(Image::ID id, void *algorithm,
  ...
  if (!db_->images().exists(id)) {
    ...
    /*		AlgorithmPipeline* detector = new AlgorithmPipeline(data.clone(),
new
Algo1Test()); // Clone to avoid thread problems TODO
       debug
                                    // Set the connection context to client, so
in case it
disconnects we
                                    // dont leak memory
                                    connect(detector,
&AlgorithmPipeline::OnFinished, client,
                                                                    [this,
client, detector](qint64 ms) {
...
this->OnColonyDetected(detector, client);
                                                                    ...
                                                                    Qt::QueuedConnection);
                                    connect(detector,
SIGNAL(OnFinished(qint64)), detector,
SLOT(deleteLater()));
FIXME
