...
namespace c3picko
{
APIController::APIController(AlgorithmManager* detector, Database* db, QObject* parent)
	: QObject(parent), detector_(detector), db_(db), input_(new APIInput(this)), output_(new APIOutput(this))
{
	/**
...
void APIController::cropImage(Image::ID id, int x, int y, int w, int h, QObject* client)
{
	if (!db_->images().exists(id))
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				AlgorithmJob* algo_job = detector_->createJob(image, algo_id, result_job_id, result, settings);
				...
				{
					connect(algo_job, &AlgorithmJob::OnFinished, client, [this, client, algo_job, result]() {
						db_->detectionResults().add(result->id(), *result); // TODO not too cool
						emit this->OnColonyDetected(&result->colonies_, client);
						qDebug() << "Detected" << result->colonies_.size() << "in" << algo_job->tookMs() << "ms";
					});
...
void APIController::startJob(Job::ID id, QObject* client)
{
	...
	DetectionResult& detection = static_cast<DetectionResult&>(db_->detectionResults().get(job.resultID()));
	...
	std::vector<Colony>					colonies = detection.colonies(); // FIXME convert to local coordinates
	std::vector<LocalColonyCoordinates> coords;
	for (int i = 0; i < colonies.size(); ++i)
		coords.push_back(Point(colonies[i].x() * 127, (1.0 - colonies[i].y()) * 85));

	auto			   code = gen.CreateGcodeForTheEntirePickingProcess(job.startingRow(), job.startingCol(), coords);
	std::ostringstream s;
