...
namespace c3picko
{
APIController::APIController(AlgorithmManager* colony_detector, AlgorithmManager* plate_detector, Database* db, QObject* parent)
	: QObject(parent), colony_detector_(colony_detector), plate_detector_(plate_detector), db_(db), input_(new APIInput(this)),
...
void APIController::UploadImage(Image image, QObject* client)
{
	...
	AlgorithmJob* ajob = plate_detector_->createJob(raw, aid, jid, result.get(), QJsonObject());
	...
	else
	{
		...
		// existing
		connect(ajob, &AlgorithmJob::OnAlgoSucceeded, client, [this, client, image, ajob, result] {
		...
		connect(ajob, &AlgorithmJob::OnAlgoFailed, client,
				[this, result, client] { emit OnImageCreateError(result->stageError(), client); });
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				std::shared_ptr<DetectionResult> result		   = std::make_shared<DetectionResult>(db_->newResultId());
				std::shared_ptr<AlgorithmJob> algo_job(colony_detector_->createJob(image, algo_id, result_job_id, result.get(), settings));
				...
				else
				{
					...
					connect(algo_job.get(), &AlgorithmJob::OnAlgoSucceeded, client, [this, client, job_id, algo_job, result] {
						db_->detectionResults().add(result->id(),
													*result); // TODO not too cool
						emit this->OnColonyDetected(job_id, &result->colonies(), client);
						qDebug() << "Detected" << result->colonies().size() << "in" << algo_job->tookMs() << "ms";
					});
					connect(algo_job.get(), &AlgorithmJob::OnAlgoFailed, client,
							[this, result, client] { emit OnColonyDetectionError(result->stageError(), client); });
					// connect(algo_job.get(), &AlgorithmJob::OnFinished, [result]() { result->cleanup(); });
...
void APIController::startJob(Job::ID id, Profile::ID octoprint_id, QObject* client)
{
	...
	DetectionResult& result = static_cast<DetectionResult&>(db_->detectionResults().get(job.resultID()));
	...
	for (QSet<Colony::ID>::iterator it = selected.begin(); it != selected.end(); ++it)
	{
