...
void APIController::UploadImage(Image image, QObject* client)
{
	...
	AlgorithmJob* ajob = plate_detector_->createJob(raw, aid, jid, result, QJsonObject());
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				std::shared_ptr<AlgorithmJob>	algo_job(colony_detector_->createJob(image, algo_id, algo_job_id, result, settings));
...
void APIController::startJob(Job::ID id, Profile::ID octoprint_id, QObject* client)
{
	if (!db_->jobs().exists(id))
	...
	PrinterProfile* printerp   = db_->profiles().get(job.printer()).operator c3picko::PrinterProfile*();
	PlateSocketProfile* socket = db_->profiles().get(job.socket()).operator c3picko::PlateSocketProfile*();
	PlateProfile* plate		   = db_->profiles().get(job.plate()).operator c3picko::PlateProfile*();
	OctoConfig* octoprint	  = db_->profiles().get(job.octoprint()).operator c3picko::pi::OctoConfig*();
	...
	{
		auto f = std::find_if(result->includedBegin(), result->includedEnd(), [&it](Colony const& c) { return c.id() == *it; });

		if (f == result->includedEnd())
			return emit OnJobStartError("Internal error: Cant find selected colonie", client);
	...
	Reporter reporter(Reporter::fromDatabase(*db_, db_->newReportId(), job.id(), pick_positions));
...
QJsonObject APIController::createImageList() const
{
	...
	std::sort(ids.begin(), ids.end(), [this](Image::ID a, Image::ID b) {
		return this->db_->images().entries()[a].uploaded() < this->db_->images().entries()[b].uploaded();
	});
