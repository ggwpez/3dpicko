...
class APIController : public QObject
{
  ...
  public:
	 ...
	 * @param parent
	 */
	APIController(AlgorithmManager* colony_detector, AlgorithmManager* plate_detector, VersionManager* version_manager, Database* db,
				  QObject* parent);
  ...
  public:
	...
	void setStartingWell(Job::ID id, Profile::ID plate_id, int row, int col, QObject* client);
	void setColoniesToPick(Job::ID id, QSet<Colony::ID> ex_user, QSet<Colony::ID> in_user, quint32 number, QObject* client);
  ...
  signals:
	...
	void OnSetStartingWell(Job::ID job, Profile::ID plate, int row, int col, QObject* client);
	void OnSetStartingWellError(QString error, QObject* client);

	void OnSetColoniesToPick(Job::ID job, QSet<Colony::ID> colonies, QObject* client);
	 ...
	 * @param client
	 */
	void OnColonyDetected(Job::ID job, std::vector<Colony> const* colonies, QObject* client);
  ...
  private:
	...
	std::shared_ptr<AlgorithmJob> detectColonies(Job::ID job_id, QString algo_id, QJsonObject settings, QObject* client);
