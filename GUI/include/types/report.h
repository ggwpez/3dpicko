...
class Report
{
  public:
	...
	/**
	 * @brief Retrives all data from the database and returns a new report filled with it.
	 * @param pick_sequence The colonies to be picked in correct order
	 */
	static Report fromDatabase(Database& db, ID id, Job::ID job, const std::map<Colony::ID, Well>& pick_positions);

	void writePdfReport(QPdfWriter*);

  private:
	Report(ID id, Job const& job, QDateTime creation, const std::map<Colony::ID, Well>& pick_positions, const Image& image,
		   DetectionResult const& results_, QSet<Colony::ID> colonies_to_pick, Profile const& plate_, Profile const& printer_,
