...
class APIOutput : public QObject
{
  ...
  protected slots:
	...
	void SetStartingWell(Job::ID job, Profile::ID plate, int row, int col, QObject* client);
	void SetStartingWellError(QString error, QObject* client);

	void SetColoniesToPick(Job::ID job, QSet<Colony::ID> colonies, QObject* client);
	 ...
	 */
	void ColonyDetected(Job::ID job, std::vector<Colony> const* detector, QObject* client);
	/**
	 * @brief Always call this from ColonyDetector or derived!
	 */
