...
HttpConnectionHandler::HttpConnectionHandler(
    QSettings *settings, HttpRequestHandler *requestHandler,
  ...
  qInfo("HttpConnectionHandler (%p): constructed", this);
  this->start();
}
...
HttpConnectionHandler::~HttpConnectionHandler() {
  quit();
  wait();
  qInfo("HttpConnectionHandler (%p): destroyed", this);
}
...
// If SSL is supported and configured, then create an instance of QSslSocket
  ...
  if (sslConfiguration) {
    ...
    socket = sslSocket;
    qInfo("HttpConnectionHandler (%p): SSL is enabled", this);
    return;
  }
#endif
...
void HttpConnectionHandler::run() {
  qInfo("HttpConnectionHandler (%p): thread started", this);
  try {
    exec();
  } catch (...) {
    qCritical(
  ...
  delete socket;
  readTimer.stop();
  qInfo("HttpConnectionHandler (%p): thread stopped", this);
}
...
void HttpConnectionHandler::handleConnection(
  ...
  if (!socket->setSocketDescriptor(socketDescriptor)) {
    qCritical("HttpConnectionHandler (%p): cannot initialize socket: %s", this,
  ...
  if (sslConfiguration) {
    qInfo("HttpConnectionHandler (%p): Starting encryption", this);
...
bool HttpConnectionHandler::isBusy() { return busy; }

void HttpConnectionHandler::setBusy() { this->busy = true; }

void HttpConnectionHandler::readTimeout() {
  qInfo("HttpConnectionHandler (%p): read timeout occured", this);
...
void HttpConnectionHandler::disconnected() {
  qInfo("HttpConnectionHandler (%p): disconnected", this);
  socket->close();
  readTimer.stop();
  busy = false;
}
...
void HttpConnectionHandler::read() {
  ...
  while (socket->bytesAvailable()) {
#ifdef SUPERVERBOSE
    qInfo("HttpConnectionHandler (%p): read input", this);
#endif
    ...
    if (currentRequest->getStatus() == HttpRequest::complete) {
      readTimer.stop();
      qInfo("HttpConnectionHandler (%p): received request", this);

      // Copy the Connection:close header to the response
      HttpResponse response(socket);
      bool closeConnection =
          QString::compare(currentRequest->getHeader("Connection"), "close",
      ...
      // This ensures that the HttpResponse does not activate chunked mode,
      // which is not spported by HTTP 1.0.
      else {
      ...
      qInfo("HttpConnectionHandler (%p): finished request", this);

      // Find out whether the connection must be closed
      if (!closeConnection) {
        // Maybe the request handler or mapper added a Connection:close header
        // in the meantime
        bool closeResponse =
            QString::compare(response.getHeaders().value("Connection"), "close",
                             Qt::CaseInsensitive) == 0;
        if (closeResponse == true) {
          closeConnection = true;
        } else {
          // If we have no Content-Length header and did not use chunked mode,
          // then we have to close the connection to tell the HTTP client that
          ...
          if (!hasContentLength) {
            bool hasChunkedMode =
                QString::compare(
                    response.getHeaders().value("Transfer-Encoding"), "chunked",
                    Qt::CaseInsensitive) == 0;
            if (!hasChunkedMode) {
              closeConnection = true;
            }
