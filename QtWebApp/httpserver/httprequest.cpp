...
#ifdef SUPERVERBOSE
    qDebug("HttpRequest: received header %s: %s", currentHeader.data(),
           value.data());
#endif
...
// received another line - belongs to the previous header
    ...
    if (headers.contains(currentHeader)) {
      headers.insert(currentHeader,
                     headers.value(currentHeader) + " " + newData);
    }
  } else {
...
void HttpRequest::readFromSocket(QTcpSocket *socket) {
  ...
  if ((boundary.isEmpty() && currentSize > maxSize) ||
      (!boundary.isEmpty() && currentSize > maxMultiPartSize)) {
...
HttpRequest::RequestStatus HttpRequest::getStatus() const { return status; }

QByteArray HttpRequest::getMethod() const { return method; }

QByteArray HttpRequest::getPath() const { return urlDecode(path); }

const QByteArray &HttpRequest::getRawPath() const { return path; }

QByteArray HttpRequest::getVersion() const { return version; }
...
QByteArray HttpRequest::getBody() const { return bodyData; }
    ...
    while (!tempFile->atEnd() && !finished && !tempFile->error()) {
      ...
      if (line.startsWith("--" + boundary)) {
        ...
        if (fileName.isEmpty() && !fieldName.isEmpty()) {
          ...
          qDebug("HttpRequest: set parameter %s=%s", fieldName.data(),
                 fieldValue.data());
...
QMap<QByteArray, QByteArray> &HttpRequest::getCookieMap() { return cookies; }
 ...
 */
QHostAddress HttpRequest::getPeerAddress() const { return peerAddress; }
