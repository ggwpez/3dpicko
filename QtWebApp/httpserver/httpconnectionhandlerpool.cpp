...
HttpConnectionHandlerPool::HttpConnectionHandlerPool(QSettings* settings, QSslConfiguration* ssl, HttpRequestHandler* requestHandler)
	: QObject(), ssl_(ssl)
{
	Q_ASSERT(settings != 0);
...
void HttpConnectionHandlerPool::loadSslConfig()
{
		...
		{
				#ifdef QT_NO_OPENSSL
						qWarning("HttpConnectionHandlerPool: SSL is not
supported"); #else
						// Convert relative fileNames to absolute, based on the
...
(QDir::isRelativePath(sslCertFileName)) #endif
						...
						{
								qCritical("HttpConnectionHandlerPool: cannot
...
open sslKeyFile %s", qPrintable(sslKeyFileName)); return;
						...
						qInfo("HttpConnectionHandlerPool: SSL settings
loaded"); #endif
		}
}*/
