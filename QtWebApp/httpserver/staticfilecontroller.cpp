...
void StaticFileController::service(HttpRequest& request,
  ...
  if (entry && (cacheTimeout == 0 || entry->created > (now - cacheTimeout))) {
    ...
    qInfo("StaticFileController: Cache miss for %s", path.data());
    // Forbid access to files outside the docroot directory
    if (path.contains("..")) {
      qWarning("StaticFileController: detected forbidden characters in path %s",
               path.data());
    ...
    qInfo("StaticFileController: Open file %s", qPrintable(file.fileName()));
    if (file.open(QIODevice::ReadOnly)) {
      ...
      if (file.exists()) {
        qWarning(
            "StaticFileController: Cannot open existing file %s for reading",
...
void StaticFileController::setContentType(QString fileName,
  ...
  if (fileName.endsWith(".png")) {
    ...
    response.setHeader("Content-Type",
                       qPrintable("text/plain; charset=" + encoding));
  } else if (fileName.endsWith(".html") || fileName.endsWith(".htm")) {
    response.setHeader("Content-Type",
                       qPrintable("text/html; charset=" + encoding));
