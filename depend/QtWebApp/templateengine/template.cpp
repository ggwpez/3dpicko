...
Template::Template(QString source, QString sourceName) : QString(source) {
...
Template::Template(QFile& file, QTextCodec* textCodec) {
  ...
  if (data.size() == 0 || file.error()) {
	qCritical("Template: cannot read from %s, %s", qPrintable(sourceName),
...
int Template::setVariable(QString name, QString value) {
  ...
  if (count == 0 && warnings) {
	qWarning("Template: missing variable %s in %s", qPrintable(variable),
...
int Template::setCondition(QString name, bool value) {
  ...
  while (start >= 0) {
	...
	if (end >= 0) {
	  ...
	  if (ellse > start && ellse < end) {
		...
		if (value == true) {
		  QString truePart =
			  mid(start + startTag.length(), ellse - start - startTag.length());
		  replace(start, end - start + endTag.length(), truePart);
		} else {
		  // value==false
		  QString falsePart =
			  mid(ellse + elseTag.length(), end - ellse - elseTag.length());
		  replace(start, end - start + endTag.length(), falsePart);
		}
	  } else if (value == true) {
		// and no else part
		QString truePart =
			mid(start + startTag.length(), end - start - startTag.length());
		replace(start, end - start + endTag.length(), truePart);
	  } else {
	  ...
	  qWarning("Template: missing condition end %s in %s", qPrintable(endTag),
			   qPrintable(sourceName));
	}
  }
  ...
  while (start >= 0) {
	...
	if (end >= 0) {
	  ...
	  if (ellse > start && ellse < end) {
		// there is an else part
		if (value == false) {
		  QString falsePart = mid(start + startTag2.length(),
								  ellse - start - startTag2.length());
		  replace(start, end - start + endTag.length(), falsePart);
		} else {
		  // value==true
		  QString truePart =
		...
		// and no else part
		QString falsePart =
			mid(start + startTag2.length(), end - start - startTag2.length());
		replace(start, end - start + endTag.length(), falsePart);
	  } else {
  ...
  if (count == 0 && warnings) {
	qWarning("Template: missing condition %s or %s in %s", qPrintable(startTag),
			 qPrintable(startTag2), qPrintable(sourceName));
  }
  return count;
}
...
int Template::loop(QString name, int repetitions) {
  ...
  while (start >= 0) {
	...
	if (end >= 0) {
	  ...
	  if (ellse > start && ellse < end) {
		...
		if (repetitions > 0) {
		  ...
		  for (int i = 0; i < repetitions; ++i) {
			...
			s.replace(QString("{if %1.").arg(name),
					  QString("{if %1.").arg(nameNum));
			s.replace(QString("{ifnot %1.").arg(name),
					  QString("{ifnot %1.").arg(nameNum));
			s.replace(QString("{else %1.").arg(name),
					  QString("{else %1.").arg(nameNum));
			s.replace(QString("{end %1.").arg(name),
					  QString("{end %1.").arg(nameNum));
			s.replace(QString("{loop %1.").arg(name),
					  QString("{loop %1.").arg(nameNum));
			insertMe.append(s);
		  }
	  ...
	  qWarning("Template: missing loop end %s in %s", qPrintable(endTag),
			   qPrintable(sourceName));
	}
  }
  if (count == 0 && warnings) {
	qWarning("Template: missing loop %s in %s", qPrintable(startTag),
			 qPrintable(sourceName));
  }
  return count;
}

void Template::enableWarnings(bool enable) { warnings = enable; }
