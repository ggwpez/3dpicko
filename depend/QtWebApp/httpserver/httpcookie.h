...
class DECLSPEC HttpCookie {
  ...
  */
  ...
  /** Set the path for that the cookie will be sent, default="/" which means the
   * whole domain */
  void setPath(const QByteArray path);

  /** Set secure mode, so that the cookie will be sent by the browser to the
   * server only on secure connections */
  void setSecure(const bool secure);

  /** Set HTTP-only mode, so that he browser does not allow client-side scripts
