...
HttpSessionStore::~HttpSessionStore() { cleanupTimer.stop(); }

QByteArray HttpSessionStore::getSessionId(HttpRequest &request,
                                          HttpResponse &response) {
  // The session ID in the response has priority because this one will be used
  // in the next request.
  mutex.lock();
  ...
  if (!sessionId.isEmpty()) {
    if (!sessions.contains(sessionId)) {
      qDebug("HttpSessionStore: received invalid session cookie with ID %s",
             sessionId.data());
      sessionId.clear();
    }
  }
...
HttpSession HttpSessionStore::getSession(HttpRequest &request,
  ...
  if (!sessionId.isEmpty()) {
    ...
    if (!session.isNull()) {
      ...
      QByteArray cookieName =
          settings->value("cookieName", "sessionid").toByteArray();
      ...
      response.setCookie(HttpCookie(cookieName, session.getId(),
                                    expirationTime / 1000, cookiePath,
                                    cookieComment, cookieDomain));
  ...
  if (allowCreate) {
    ...
    qDebug("HttpSessionStore: create new session with ID %s",
           session.getId().data());
