...
QMap<QByteArray, QByteArray> &HttpResponse::getHeaders() { return headers; }
...
void HttpResponse::write(QByteArray data, bool lastPart) {
  ...
  // Send HTTP headers, if not already done (that happens only on the first call
  // to write())
  if (sentHeaders == false) {
    // If the whole response is generated with a single call to write(), then we
    // know the total size of the response and therefore can set the
    ...
    // else if we will not close the connection at the end, them we must use the
    // chunked mode.
    else {
      QByteArray connectionValue =
          headers.value("Connection", headers.value("connection"));
      bool connectionClose =
          QString::compare(connectionValue, "close", Qt::CaseInsensitive) == 0;
      if (!connectionClose) {
...
bool HttpResponse::hasSentLastPart() const { return sentLastPart; }
...
QMap<QByteArray, HttpCookie> &HttpResponse::getCookies() { return cookies; }
