...
static Colony makeColony(int x, int y) { return Colony(x, y, 0, 0, 0, 0, 0, ""); }
...
void ColonyCollision::initTestCase()
{
	 ...
	 *    O O
	 */
	solutions_.push_back({{makeColony(0, 0), makeColony(r * 2 + p, 0), makeColony(0, r * 2 + p), makeColony(r * 2 + p, r * 2 + p),
	...
	solutions_.push_back(
		{{makeColony(0, 0), makeColony(r * 2 + p, 0), makeColony(0, r * 2 + p), makeColony(r * 2 + p, r * 2 + p)}, r, d, 2});
...
void ColonyCollision::test_case1()
{
	for (std::size_t i = 0; i < solutions_.size(); ++i)
	{
		std::vector<Colony> result
			= math::filterByMinDistanceSlow(solutions_[i].colonies_, solutions_[i].r, solutions_[i].d, solutions_[i].min_num_);

		qDebug().nospace() << "TEST #" << i << "  (n=" << solutions_[i].colonies_.size() << ")";
