...
template <typename T> struct Range {
  inline Range(Range const &other)
      : lower_(other.lower_), upper_(other.upper_),
        lower_closed_(other.lower_closed_), upper_closed_(other.upper_closed_) {
  }
  inline Range()
  ...
  inline Range(T const &lower, T const &upper, bool lower_closed = true,
      ...
      : lower_(lower), upper_(upper), lower_closed_(lower_closed),
        upper_closed_(upper_closed) {}

  template <typename S> inline bool contains(S const &value) const {
  ...
  template <typename S> inline bool excludes(S const &value) const {
