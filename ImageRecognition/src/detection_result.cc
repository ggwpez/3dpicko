...
{
	QJsonObject obj = Marshalling::toJson<AlgorithmResult>(static_cast<AlgorithmResult const&>(value));
	QJsonArray  colonies;
...
DetectionResult::DetectionResult(AlgorithmResult&& base) : MatrixResult(std::move(base)) {}
...
void DetectionResult::finalize()
{
	...
	auto it		  = std::partition(colonies_.begin(), colonies_.end(), std::bind(&Colony::included, std::placeholders::_1));
...
std::vector<Colony>::iterator DetectionResult::excludedBegin() { return includedEnd(); }
...
const std::vector<Colony>& DetectionResult::colonies() const { return colonies_; }
