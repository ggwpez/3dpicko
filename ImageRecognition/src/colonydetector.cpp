...
namespace c3picko
{
	...
	void ColonyDetector::drawText(cv::Mat& img, std::vector<cv::Vec3f>& colonies)
	{
		...
		for (int i = 0; i < colonies.size(); ++i)
		{
			...
			cv::putText(stages_.back(), text, t_pos, f_type, f_scale, cv::Scalar::all(255), f_thick);
		}
	}

	bool ColonyDetector::roundness(cv::Mat& out, int area, int w, int h, std::vector<cv::Point> const& contour, double roundness)
	{
	...
	void ColonyDetector::run()
	{
		...
		for (std::size_t i = 0; i < colonies_.size(); ++i)
			pos_[i] = cv::Vec3d(colonies_[i].x(), colonies_[i].y(), colonies_[i].circumference() /(2*M_PI));
	...
	{
		Q_ASSERT(stages_.size() == 0 && colonies_.size() == 0 && "Dont call more than once!");
			...
			for(int c = 0; c < labeled_stage.cols; ++c)
			{
				...
				else
				{
					...
					else if (label)
					{
						...
						{
							Colony detected(center.x /double(source_.cols), center.y /double(source_.rows), area, cv::arcLength(contours[0], true), 0, 0, ++id, Colony::Type::INCLUDED);
	...
	std::vector<Colony> ColonyDetector::colonies() const
	{
		return colonies_;
	}

	std::vector<cv::Vec3f> ColonyDetector::pos() const
	{
		return pos_;
	}
}
