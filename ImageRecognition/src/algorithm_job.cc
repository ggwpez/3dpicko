...
namespace c3picko
{
AlgorithmJob::AlgorithmJob(AlgorithmJob::ID id, Algorithm* algo, QJsonObject settings, void* input, AlgorithmResult* result,
						   QThreadPool* pool, qint64 max_ms, QObject* _parent)
	: QObject(_parent), id_(id), created_(QDateTime::currentDateTime()), algo_(algo), pool_(pool), input_(input), result_(result),
	  result_id_(result->id()), max_ms_(max_ms), took_ms_(0)
{
