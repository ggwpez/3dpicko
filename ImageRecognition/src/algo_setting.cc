...
AlgoSetting AlgoSetting::make_checkbox(ID id, QString name, QString description, bool default_value, QList<AlgoSetting> sub_settings,
									   QColor color)
{
	return AlgoSetting(id, name, "checkbox", description, empty_qv, empty_qv, empty_qv, {}, default_value, sub_settings, color);
}

AlgoSetting AlgoSetting::make_dropdown(AlgoSetting::ID id, QString name, QString description, QVariantMap options,
									   QString default_option_index, QColor color)
{
	return AlgoSetting(id, name, "dropdown", description, empty_qv, empty_qv, empty_qv, options, default_option_index, {}, color);
}

AlgoSetting AlgoSetting::make_rangeslider_double(AlgoSetting::ID id, QString name, QString description, double min, double max,
												 double step, math::Range<double> default_value, QColor color)
{
	return AlgoSetting(id, name, "rangeslider", description, min, max, step, {}, QVariant::fromValue(default_value), {}, color);
}
...
AlgoSetting::AlgoSetting(AlgoSetting::ID id, QString name, QString type, QString description, QVariant min, QVariant max, QVariant step,
						 QVariantMap options, QVariant default_value, QList<AlgoSetting> sub_settings, QColor color, QVariant value)
	: id_(id), name_(name), type_(type), description_(description), min_(min), max_(max), step_(step), options_(options),
	  default_value_(default_value), sub_settings_(sub_settings), color_(color), value_(value)
{
	if (type == "slider")
	{
		...
		if ((min_.userType() != max_.userType()) || (max_.userType() != step_.userType())
			...
			throw Exception("AlgoSetting: All passed values must have the declared type");
	}
	// TODO else

	if (value_.isValid() && (value_.userType() != default_value_.userType()))
		throw Exception("AlgoSetting: default_value should have always the same type as value");
}
...
void AlgoSetting::addSubSetting(const AlgoSetting& sub) { sub_settings_.push_back(sub); }
...
template <> Q_DECL_DEPRECATED AlgoSetting Marshalling::fromJson(const QJsonObject& obj)
{
	...
	else if (type == "checkbox")
	{
		if (!default_value.canConvert<bool>())
			throw Exception("Could not convert default value for checkbox to boolean");
