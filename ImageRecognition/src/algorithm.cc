...
void Algorithm::run()
{
	...
	QTextStream(&error_prefix) << "Algorithm " << this->metaObject()->className() << " crashed (step=";
	QTextStream(&error_postfix) << ",job=" << job->id() << ",thread=" << QThread::currentThreadId() << ") ";
...
Algorithm::Algorithm(Algorithm::ID id, QString name, QString description, QList<AlgoStep> steps, QList<AlgoSetting> settings,
	...
	: id_(id), name_(name), description_(description), steps_(steps), default_settings_(settings), is_threadable_(is_threadable),
	  max_ms_(max_ms)
{
}
...
QList<AlgoSetting> Algorithm::defaultSettings() const { return default_settings_; }
