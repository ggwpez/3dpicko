...
void Normal1::label(AlgorithmJob* base, DetectionResult* result)
{
	...
	for (int i = 0; i < centers.rows; ++i)
	{
		...
		{
			if (show_excluded)
				colonies.emplace_back(center.x / double(input.cols), center.y / double(input.rows), 0, 0, 10, 0, ++id,
		...
		{
			colonies.emplace_back(center.x / double(input.cols), center.y / double(input.rows), area, 0, std::sqrt(area / M_PI), 0, ++id,
								  "area");
			continue;
		}
		// Filter by roundness
