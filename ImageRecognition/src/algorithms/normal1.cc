...
Normal1::Normal1()
	: Algorithm("1", "Normal1", "Detects colonies with standard illumination",
				{(AlgoStep)&Normal1::cvt, (AlgoStep)&Normal1::threshold, (AlgoStep)&Normal1::erodeAndDilate, (AlgoStep)&Normal1::label,
				 (AlgoStep)&Normal1::relativeFiltering},
				{AlgoSetting::make_checkbox("show_excluded_by_algo", "Show ignored by algorithm", "", true, {}, Qt::red),
				 AlgoSetting::make_rangeslider_double("area", "Area", "lel", 10, 2000, 1, {120, 1000}),
...
void Normal1::drawText(cv::Mat& img, cv::Mat& output, std::vector<cv::Vec3f>& colonies)
{
...
void Normal1::label(AlgorithmJob* base, DetectionResult* result)
{
	...
	for (int i = 0; i < centers.rows; ++i)
	{
		...
		if (!label)
		{
			if (show_excluded)
				colonies.push_back(
					Colony(center.x / double(input.cols), center.y / double(input.rows), 0, 0, 10, 0, ++id, "show_excluded_by_algo"));
			continue;
		}
		...
		{
			colonies.push_back(
				Colony(center.x / double(input.cols), center.y / double(input.rows), area, 0, std::sqrt(area / M_PI), 0, ++id, "area"));
			continue;
		}
		// Filter by roundness
		else
		{
			...
			{
				colonies.push_back(Colony(center.x / double(input.cols), center.y / double(input.rows), area, 0, std::sqrt(area / M_PI), 0,
										  ++id, "aabb_ratio"));
			}
