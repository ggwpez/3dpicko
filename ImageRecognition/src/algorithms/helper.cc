...
namespace math
{
	...
	void findConnectedComponentEdges(cv::Mat const& input, std::vector<std::vector<cv::Point>>& contours, math::Range<int> const& area)
	{
	...
	bool polyIncludesPoly(std::vector<cv::Point> const& poly1, std::vector<cv::Point> const& poly2)
	{
