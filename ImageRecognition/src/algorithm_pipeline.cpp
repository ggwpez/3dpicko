...
namespace c3picko
{
AlgorithmPipeline::AlgorithmPipeline(QThreadPool* pool, QList<Algorithm*> algos, QObject* _parent)
...
AlgorithmJob* AlgorithmPipeline::createJob(cv::Mat source, Algorithm::ID id, QJsonObject settings)
{
	for (Algorithm* algo : algos_)
	{
		if (algo->id() == id)
