		...
		ConnectionInfo::ConnectionInfo(QJsonObject obj)
		{
			...
			for (auto profile : options_json["printerProfiles"].toArray())
				options.printer_profiles.insert(
					{profile.toObject()["name"].toString(), profile.toObject()["id"].toString()}); // QtJson is dumb
			...
			options.printer_profile_preference = options_json["printerProfilePreference"].toString();
