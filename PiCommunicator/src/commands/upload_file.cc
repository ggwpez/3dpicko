...
UploadFile::UploadFile(QByteArray data, QString path, QString file_name,
					   bool select, bool print)
	: Command("files/" + path,
			  BuildFileUploadPacket(data, file_name, select, print), {201}, Command::Type::POST,
			  "multipart/form-data; boundary=" + boundry1) {}

UploadFile::UploadFile(QString folder_name, QString path)
	: Command("files", BuildFolderCreatePacket(folder_name, path), {201}, Command::Type::POST,
			  "multipart/form-data; boundary=" + boundry2) {}

QByteArray UploadFile::BuildFileUploadPacket(QByteArray data, QString file_name,
											 bool select, bool print) {
  QByteArray query;
