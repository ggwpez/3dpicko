...
namespace pi
{
	namespace commands
	{
		...
		{
			return new ArbitraryCommand("printer/command", QJsonObject({{"command", command}}), // ugly
										{204}, Command::HTTPType::POST);
		}

		ArbitraryCommand* ArbitraryCommand::MultiCommand(QStringList commands)
		{
			return new ArbitraryCommand("printer/command", QJsonObject({{"commands", QJsonArray::fromStringList(commands)}}), // ugly
										{204}, Command::HTTPType::POST);
		}
	}
}
